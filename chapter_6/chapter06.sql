...
CREATE TABLE
    departments (
        ...
        CONSTRAINT dept_city_unique UNIQUE (cept, city)
    );
...
CREATE TABLE
    school_enrollment (id integer, enrollment integer);

CREATE TABLE
    school_grades (id integer, grades varchar(10));

INSERT INTO
    school_enrollment (id, enrollment)
VALUES
    (1, 360),
    (2, 1001),
    (5, 450),
...
FROM
    schools_left AS lt
    LEFT JOIN school_enrollment AS en ON lt.id = en.id
    LEFT JOIN school_grades AS gr ON lt.id = gr.id;
